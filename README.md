...
- **Rich tooltips** — every builder action button (Answer Type, Required, Show When, Applicability, Expression, Default, Appearance) and the Variables card title show a tooltip with a FHIR field path and spec reference (R4 / SDC) in the footer; `js/ui/tooltip.js` uses delegated `mouseover` — no per-element registration; dark card positions below or above target; no native `title=` flicker
