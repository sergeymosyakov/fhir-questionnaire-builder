...
- **Vitest** — unit test suite for pure-function modules (`utils`, `eval`, `fhir/calc`, `fhir/validate`, `fhir/export`, `fhir/import`, `fhir/qr-builder`); 188 tests; CDN imports mocked via `vi.mock`; CI via GitHub Actions (`npm test`)
...
- **Expression Explain** — click a checkbox calc-badge or `👁️`/`🔒` condition-hint badge (when `enableWhenExpression` is set) to open a centered popup that breaks the FHIRPath expression into an AND/OR/NOT/LEAF tree and shows ✓/✗ next to each node
