...
16. **No silent workarounds in test helpers** — `if (await modal.isVisible()) { await modal.close() }` in a setup helper is forbidden unless the test explicitly covers that modal. If something unexpected opens during test setup, fix the root cause.
