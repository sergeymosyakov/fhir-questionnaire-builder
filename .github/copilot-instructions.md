...
4. **New rules go into the repo, not just memory** — whenever a new rule is established (in conversation, from a bug, from a lesson learned), add it immediately to the correct customization file (see layout below), without asking. Do not store rules only in local memory.
