...
- **Simple/Advanced view mode is a pure CSS layer** — `SimpleMode` (`js/ui/simple-mode.js`) only toggles the `builder--simple` class on `[data-mount="left-panel"]` and persists the choice; it never re-renders the builder. Any mode-conditional UI must therefore be **rendered always** and shown/hidden via CSS (`.builder--simple .foo { display: … }`) — **never** re-render nodes or branch on the mode inside `buildBuilder()` in response to `BUILDER_VIEW_MODE_CHANGE`. Advanced-only node action links (the ones that open modals) carry `data-role="advanced-ctrl"` (set by `makeActionLink`) and are hidden in Simple mode by a single CSS rule.
- **Per-node actions live in the ⚙ gear menu** — add/delete/copy/paste and similar node-level actions belong in the node's `NodeGearMenu` (`js/ui/node-gear-menu.js`), not as standalone title-row icons. Add new per-node actions via `gear.addItem(label, testid, onClick, {destructive?, disabled?})`. Core node *attributes* (title, answer type) may still render inline in the card — the gear menu is for *actions*, not fields.
