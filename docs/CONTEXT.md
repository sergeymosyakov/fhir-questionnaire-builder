...
| `js/nodes/item-node.js` | `ItemNode extends BaseNode` — all non-group, non-display item types; overrides `_evalCondition`, `_buildLabel`, `_buildRowContent`, `_buildConstraintBadge`, `_buildCalcBadge`, `_appendRow`; **`buildBuilder(ctx)`** — renders the builder-panel row for this item (DnD, action links, modals via `MODAL_REGISTRY`); all builder deps injected through `ctx`; **`supportsRepeat() → true`** — overridden by `CheckboxNode` and `DisplayNode` to return `false`; `buildBuilder` uses `!node.supportsRepeat()` to hide the repeat action link; **`impliesRepeats() → false`** — overridden by `ChecklistNode` to return `true` (multi-select check-box is inherently `repeats:true`); `_buildControl` renders the "+ Add another" repeat container only when `repeats && supportsRepeat()`, so checklist/checkbox never show it |
