...
| `js/state.js` | Reactive state, data factories, business logic, `evalConstraints`; exports `questMeta` reactive object (id, url, version, name, title, status, publisher, description, date, subjectType, purpose, copyright, approvalDate, lastReviewDate, _rawContact, _rawUseContext, _rawJurisdiction); `CHECKABLE_TYPES` and `NONEMPTY_TYPES` include `time` and `dateTime` (mandatory items of these types participate in PASS/FAIL validation) |
...
| `js/ui/metadata-modal.js` | Questionnaire Properties modal — `init(elements)`, `open()`; draft pattern; edits all `questMeta` fields via two sections: **Core** (always visible: id, url, version, name, title, status dropdown, publisher, description) and **Advanced** (collapsible, collapsed by default: date, subjectType, approvalDate, lastReviewDate, purpose, copyright); toggle button labeled `▶ Advanced` / `▼ Advanced`; changes committed on Apply; reflected in compact `questMetaCard` (non-collapsible panel above Variables card) |
| `js/ui/codes-modal.js` | Item Codes modal — `init(elements)`, `open(node, link, setActive)`; draft pattern; edits `node._codes[]` (FHIR `item.code[]`); each row has system URL, code, and display inputs + × remove; `+ Add code` button; Apply filters empty codes and commits; Cancel discards; action button highlighted when `_codes` non-empty |
...
| `tests/e2e/metadata-modal.spec.js` | E2E tests (27) — questMetaCard visibility, status badge (color-coded draft/active/retired), modal open/close, fields pre-populated from loaded questionnaire (incl. name), Advanced section toggle (collapsed by default → expands on click), Advanced field values (date, subjectType, purpose), Advanced fields round-trip export (subjectType, date, contact pass-through), Apply commits/Cancel discards, export round-trip with changed title, reset of all fields on form clear; fixture `tests/fixtures/meta-test.fhir.json` |
...
- **Playwright** — E2E test suite; **233 tests** across 17 spec files (Chromium); CI via GitHub Actions (`npx playwright test`)
...
- **Vitest** — unit test suite for pure-function modules; **364 tests** across 10 files; CDN imports mocked via `vi.mock`; CI via GitHub Actions (`npm test`)
