...
| `js/ui/menus/questionnaires-menu.js` | `QuestionnairesMenu extends DropdownMenu` — Load/Library/Cloud/Recent-draft items; `_onOpen` syncs recent-draft item; hidden `#fhirFileInput` for file picker |
| `js/ui/menus/answers-menu.js` | `AnswersMenu extends DropdownMenu` — Load QR file / Load from Library; hidden `#qrFileInput`; initially hidden, shown when tree has nodes |
| `js/ui/menus/save-menu.js` | `SaveMenu extends DropdownMenu` — Cloud Save / Questionnaire JSON / QR JSON; `setHandlers({onExportFhir, onExportQr})` late binding from `app.js` |
