...
| `js/ui/modals/sdc-populate-modal.js` | `SdcPopulateModal extends Modal` — patient reference input; auto-prefixes bare IDs with `Patient/`; on Apply dispatches `AppEvents.SDC_POPULATE_REQUESTED { patientRef }`; `testid: sdcPopulate` |
| `js/ui/sdc-populate-btn.js` | `SdcPopulateBtn` — self-wiring; mounts to `[data-mount="sdc-populate-btn"]`; visible only when `fhirBaseUrl` configured in `serverConfig` AND questionnaire is loaded; click opens `sdcPopulateModal` |
...
| `js/ui/menus/answers-menu.js` | `AnswersMenu extends DropdownMenu` — Load QR file / Load from Library; hidden file input (`data-testid="qr-file-input"`); initially hidden, shown when tree has nodes; fully event-driven |
