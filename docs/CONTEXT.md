...
- **Appearance "Copy to…"** — **Copy to…** button in the Appearance modal footer opens the **Node Picker modal** (tree with checkboxes, search, multi-select); on confirm dispatches `AppEvents.COPY_TO_NODES {ids, patch}` — every `BaseNode` listener applies `applyPatch()` to matching nodes (null = delete field, value = assign); propagates `_renderStyle` / `_renderXhtml` / `_renderMarkdown` to any other nodes at once (`js/ui/modals/node-picker-modal.js`)
