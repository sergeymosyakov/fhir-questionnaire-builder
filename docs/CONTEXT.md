...
| `js/fhir/terminology-service.js` | Singleton `terminologyService` — `getServer(node, questMeta)` (fallback chain: per-item → questionnaire → `https://tx.fhir.org/r4`); `expandValueSet(vsUrl, serverUrl)` fetches `ValueSet/$expand`; `testServer(serverUrl)` pings `/metadata`; `expandAll(tree, questMeta)` walks tree, stores options in `node._vsCache`, returns failures array |
