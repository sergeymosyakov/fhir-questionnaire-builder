...
| `js/fhir/format-registry.js` | Unified `FormatRegistry` singleton — single source of truth for all export formats (FHIR R4/R4B/R5, REDCap CSV). API: `register(def)`, `get(id)`, `getAll()`, `getBuilderVersions()` (only `isBuilderVersion:true` entries), `detectVersion(data)` (reads the `builder-target-version` extension first, then feature heuristics: native `disabledDisplay`→R5, `answerConstraint`→R4B, else R4). Also exports `BUILDER_VERSION_EXTENSION_URL` and `setBuilderVersion(q, code)` helper. Format def shape: `id`, `label`, `selectorLabel?`, `isBuilderVersion`, `metaVersion?`, `ext`, `mimeType`, `build(baseQ)`, `reportTitle?`, `validatorFilter?`, `onBeforeReport?`, `onAfterExport?`, `onCancel?` |
| `js/fhir/formats/r4.js` | Self-registers R4 format into `formatRegistry` — `isBuilderVersion:true`, `metaVersion:'4.0.1'`, `selectorLabel:'FHIR R4'`, `build()` stamps the `builder-target-version` extension `valueCode:'4.0.1'` |
| `js/fhir/formats/r4b.js` | Self-registers R4B format — `isBuilderVersion:true`, `metaVersion:'4.3.0'`, `selectorLabel:'FHIR R4B'`, `build()` stamps the `builder-target-version` extension `valueCode:'4.3.0'` |
| `js/fhir/formats/r5.js` | Self-registers R5 format — `isBuilderVersion:true`, `metaVersion:'5.0.0'`, `selectorLabel:'FHIR R5'`, `build()` converts `open-choice→choice+answerConstraint:optionsOrString` recursively, stamps the `builder-target-version` extension `valueCode:'5.0.0'` |
...
| `js/fhir/validators/external.js` | `ExternalValidator extends Validator` — `id='external'`; `enabled=false` by default; POSTs to HAPI FHIR `$validate` endpoint via CORS proxy (`corsProxyUrl` from config.json); retries configurable on network/5xx errors only; non-retryable 4xx (incl. HTTP 413 Payload Too Large → clear "questionnaire too large" message) raised as `ValidatorFatalError`; maps OperationOutcome issues to `{ severity, nodeId: '(external)', message }` |
