...
| `js/preview-form.js` | Right panel — `PreviewForm` class; reactive preview driven by direct `AppEvents` listeners (`RESPONSE_CHANGED`, `REINIT_FORM`, etc.); `reinitForm({ silent? })` rebuilds form (progress bar suppressed when `silent:true` — used for background ValueSet expansion re-evals); exports `getLastCtx()` compat shim |
