...
| `js/render-preview.js` | Right panel — reactive preview; `initPreview(elements)` called once from app.js to inject DOM refs (DI pattern); `_applyRenderStyle(el, raw)` — safe allowlist parser for `node._renderStyle` (permits only `font-weight`, `font-style`, `color`, `font-size`, `text-decoration`; discards any other property); **Preview mode dropdown** (`previewMode.value`; `'preview'|'patient'|'json'`): `#previewModeWrap` dropdown with three options; **Patient** mode skips hidden items, nav buttons, badges, condition hints, ok/fail icons — renders a clean form as a patient sees it; **FHIR JSON** mode hides `#lform`, shows `<pre id="fhirJsonView">` with syntax-highlighted JSON (via `highlightJson`); search works in JSON mode (highlights `<mark>` via `highlightJsonWithSearch`); **Hidden items** (`sdc-questionnaire-hidden`): rendered with purple dashed border + striped background + **HIDDEN badge** in builder preview when `showHiddenItems` toggle is on; for **groups** the border + stripes wrap a `div.lform-item--hidden` that spans the header row AND nested children; for items the class is applied directly to the row; controls within hidden items disabled; hidden items excluded from PASS/FAIL validation; invisible to patients; `buildRepeatControls` renders multi-row repeat UI; enforces `node._maxOccurs` — add button disabled at limit; `hasCriteria` includes `hasConstraints`+`hasRange` so badge is visible for constraint/range-only questionnaires; group icon `relevantItems` filter and `itemOk` predicate include constraint and range items; `display` items with `_displayCategory` get `.lform-item--{category}` class + icon or help-toggle |
