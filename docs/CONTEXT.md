...
| `js/nodes/group-node.js` | `GroupNode extends BaseNode` — group nodes; overrides `_evalCondition`, `_buildLabel`, `_buildRowContent`, `_renderChildren`, `_renderDimmedChildren`, `_renderDisabledChildren`; **`refreshIcon(rc)`** — updates pass/fail icon; **`buildBuilder(ctx)`** — renders the builder-panel row for this group (collapse toggle, child list, ⚙ gear menu — Add Group / Add Item / Delete — DnD, action links, style panel, modals via `MODAL_REGISTRY`); `destroy()` overrides `super.destroy()` and recursively calls `c.destroy()` on all children |
...
| `js/ui/node-gear-menu.js` | `NodeGearMenu` — ⚙ gear dropdown in the top-right of a builder node (replaces the old `×` delete button and the `⊕ Add ▾` menu). `addItem(label, testid, onClick, {destructive})` + `addSep()`; follows the `CLOSE_DROPDOWNS` toggle pattern. Groups get **Add Group / Add Item / Delete** (gear button keeps `data-testid="group-add-btn"`); items get **Delete** only (`data-testid="node-gear-btn"`). The gear is not `advanced-ctrl`, so it stays visible in Simple mode |
