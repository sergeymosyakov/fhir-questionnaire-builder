...
| `js/app.js` | Entry point — wires inputs, buttons, loads example; calls `initPreview({lform, showLinkIdBtn, showPrefixBtn, showBadgesBtn, showHiddenBtn, patientViewBtn, previewCollapseAllBtn, previewExpandAllBtn, searchWrap, leftPanelBody})` to inject DOM references into render-preview.js |
| `js/state.js` | Reactive state, data factories, business logic, `evalConstraints`; exports `patientMode` ref (toggles patient view in preview); exports `showHiddenItems` ref (toggles visibility of `sdc-questionnaire-hidden` items in preview); exports `questMeta` reactive object (id, url, version, name, title, status, language, publisher, description, date, subjectType, purpose, copyright, approvalDate, lastReviewDate, effectivePeriodStart, effectivePeriodEnd, experimental, _rawContact, _rawUseContext, _rawJurisdiction, _rawCode); `CHECKABLE_TYPES` and `NONEMPTY_TYPES` include `time` and `dateTime` (mandatory items of these types participate in PASS/FAIL validation) |
...
| `js/ui/metadata-modal.js` | Questionnaire Properties modal — `init(elements)`, `open()`; draft pattern; edits all `questMeta` fields via three sections: **Core** (always visible: id, url, version, name, title, status dropdown, **language BCP-47 dropdown**, publisher, description), **Advanced** (collapsible, collapsed by default: **experimental select (not-set/Yes/No)**, date, subjectType, effectivePeriodStart/End, approvalDate, lastReviewDate, purpose, copyright), **Codes** (collapsible: edits `Questionnaire.code[]` via shared `renderCodesEditor`; badge shows code count), **Derived From** (collapsible: canonical URL list), **Resource Meta** (collapsible: `meta.versionId` text input + **Generate UUID** button; `meta.source` editable URI input; `meta.lastUpdated` read-only display always refreshed on export; `meta.profile[]` URL list; `meta.tag[]` and `meta.security[]` Coding rows via `renderCodesEditor`); `_makeRow(key, label, type, placeholder, testid, tip?)` and `_makeSelectRow(key, label, options, testid, tip?)` helpers accept optional `tip` object — when provided, sets `data-tip-title/body/fhir/spec` on the field label; **all field labels and all four section toggle buttons carry rich tooltips** (`Questionnaire.*` R4 paths); `_makeSelectRow()` helper renders select with custom imported-value fallback; toggle button labeled `▶ Advanced` / `▼ Advanced`; changes committed on Apply; status + experimental badge reflected in compact `questMetaCard` |
| `js/ui/codes-modal.js` | **Item / Group Properties** modal — `init(elements)`, `open(node, link, setActive)`; draft pattern; three collapsible sections: **Definition URL** (`node._definition`), **Codes** (`node._codes[]` — system/code/display rows), **Support Links** (`node._supportLinks[]` — URL inputs, 0..*); Apply commits all three fields; Cancel discards; action button highlighted when any field is non-empty; shared `renderCodesEditor(draft, container, prefix, label?)` exported for reuse — optional `label` parameter customises empty-state text and add-button label (default `'code'`); available on both items and gro...
...
| `tests/eval.test.js` | Unit tests for `js/eval.js` — `evaluateNode`, `markAllDisabled`, `enableWhen` AND/OR logic, `_hidden`/`hiddenRoot` marking (28 tests) |
...
- **Playwright** — E2E test suite; **264 tests** across 19 spec files (Chromium); CI via GitHub Actions (`npx playwright test`)
...
- **Vitest** — unit test suite for pure-function modules; **443 tests** across 10 files; CDN imports mocked via `vi.mock`; CI via GitHub Actions (`npm test`)
