...
| `js/builder/node-item.js` | `renderItem(node, ctx)` — opens `answer-type-modal`, `showwhen-modal`, `expression-modal` (via `openDual` — merged Expression+InitExpr button), `constraint-modal`, `initial-modal`, `appearance-modal`, `required-modal`, `repeatable-modal` for all action links; actions row rendered on its own row below the title row (no overlap with × delete); no inline panels remain |
| `js/builder/node-group.js` | `renderGroup(node, ctx)` — opens `showwhen-modal`, `expression-modal` (via `open(cfg)` — single calculatedExpression only), `required-modal` for respective action links; actions row on its own row below title; `style` still uses inline `buildStylePanel` |
| `js/render-preview.js` | Right panel — reactive preview; **Patient View** mode (`patientMode.value`): skips hidden items, nav buttons, badges, condition hints, ok/fail icons — renders a clean form as a patient sees it; `buildRepeatControls` renders multi-row repeat UI; enforces `node._maxOccurs` — add button disabled at limit; `hasCriteria` includes `hasConstraints`+`hasRange` so badge is visible for constraint/range-only questionnaires; group icon `relevantItems` filter and `itemOk` predicate include constraint and range items |
...
| `js/ui/codes-modal.js` | Item Properties modal — `init(elements)`, `open(node, link, setActive)`; draft pattern; **Definition URL** field (`node._definition`); collapsible **Codes** section (always expanded on open) edits `node._codes[]` (FHIR `item.code[]`); each row has system URL, code, display inputs + × remove; `+ Add code` button; Apply commits filtered codes + definition; Cancel discards; action button highlighted when `_codes` non-empty or `_definition` set; exports shared `renderCodesEditor(draft, container, prefix)` for reuse in other modals |
...
- **Vitest** — unit test suite for pure-function modules; **409 tests** across 10 files; CDN imports mocked via `vi.mock`; CI via GitHub Actions (`npm test`)
