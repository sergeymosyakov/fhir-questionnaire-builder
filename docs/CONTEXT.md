...
| `js/builder/node-item.js` | `renderItem(node, ctx)` — opens `answer-type-modal`, `showwhen-modal`, `expression-modal` (via `openDual` — merged Expression+InitExpr button), `constraint-modal`, `initial-modal`, `appearance-modal`, `required-modal`, `repeatable-modal`, `codes-modal` (Props button — edits `_definition`, `_codes[]`, `_supportLinks[]`) for all action links; actions row rendered on its own row below the title row (no overlap with × delete); no inline panels remain |
| `js/builder/node-group.js` | `renderGroup(node, ctx)` — opens `showwhen-modal`, `expression-modal` (via `open(cfg)` — single calculatedExpression only), `required-modal`, `codes-modal` (**Props** button — edits `_definition`, `_codes[]`, `_supportLinks[]`) for respective action links; actions row on its own row below title; `style` still uses inline `buildStylePanel` |
| `js/render-preview.js` | Right panel — reactive preview; **Patient View** mode (`patientMode.value`): skips hidden items, nav buttons, badges, condition hints, ok/fail icons — renders a clean form as a patient sees it; **Hidden items** (`sdc-questionnaire-hidden`): rendered with purple dashed border + **HIDDEN badge** in builder preview when `showHiddenItems` toggle is on; controls within hidden items disabled; hidden items excluded from PASS/FAIL validation; invisible to patients; `buildRepeatControls` renders multi-row repeat UI; enforces `node._maxOccurs` — add button disabled at limit; `hasCriteria` includes `hasConstraints`+`hasRange` so badge is visible for constraint/range-only questionnaires; group icon `relevantItems` filter and `itemOk` predicate include constraint and range items; `display` items with `_displayCategory` get `.lform-item--{category}` class + icon or help-toggle |
