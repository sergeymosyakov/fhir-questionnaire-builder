...
| `js/ui/modals/save-format-modal.js` | `SaveFormatModal extends Modal` — opened by `SaveMenu` on "Questionnaire…" click; format dropdown (FHIR R4 JSON / REDCap CSV); FHIR path: runs `validateModal('export')` then `showPrompt` for filename; REDCap path: enables `redcapCompatValidator`, runs `validateModal('export')`, downloads CSV; remembers last format in `localStorage` | — `show(title, mode, { questJson, tree, values, onExport?, extraIssues? })`; `mode`: `'export'` runs all validators first (disabled ones return `[]` automatically), skips modal entirely if 0 issues, otherwise opens pre-filled (no spinners); `'validate'|'import'` opens immediately with spinners, only enabled validators shown; footer: `'export'` → Fix first + Export/Export anyway; `'validate'|'import'` → OK/Great! + "— All good" title suffix when clean |
