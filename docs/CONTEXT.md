...
| `js/app.js` | Pure composition root — creates data singletons (`questDoc`, `answerStore`), instantiates top-level UI components (all self-find DOM via `[data-mount="..."]`), dispatches `APP_CONTEXT_READY`. No DOM queries, no callbacks, no `configure()` calls. All modules self-wire via events or `EventState`. |
| ~~`js/state.js`~~ | **DELETED** — was a barrel re-export. `questDoc` → import from `./fhir/quest-document.js`; `answerStore` → import from `./answer-store.js`; form-check functions → import from `./fhir/form-checks.js` |
| `js/preview-form.js` | Right panel — `PreviewForm` class; preview driven by `AppEvents` listeners (`RESPONSE_CHANGED`, `REINIT_FORM`, `SDC_POPULATE_REQUESTED`, etc.); `reinitForm({silent?})` rebuilds form; `_populate(patientRef)` calls `populateFromServer()` → `importQRAnswers()` → `REINIT_FORM` on success; exports `getLastCtx()` compat shim |
| `js/fhir/sdc-populate.js` | Pure `populateFromServer(fhirBase, questJson, patientRef, corsProxy?)` — `POST Questionnaire/$populate` with `Parameters` body; handles both direct `QuestionnaireResponse` result and wrapped `Parameters` result; 30s timeout; CORS proxy-aware |
| `js/fhir/fhir-search.js` | Pure FHIR REST search helpers shared by `ReferenceNode` and `SdcPopulateModal`: `searchFhir(resourceType, query)` (live name search, reads `OperationOutcome` diagnostics on error), `displayName(resource)` (Patient/Practitioner/Org/Encounter/Condition), `proxiedUrl(url)` (skips CORS proxy for hosts in `CORS_ENABLED_HOSTS`, exact `hostname` match) |
| `js/fhir/sdc-definition-extract.js` | Pure `definitionExtract(questJson, qr) → { bundle, count, warnings }` — walks groups carrying `sdc-questionnaire-definitionExtract`, maps child `item.definition` answers to FHIR resource element paths, emits a transaction `Bundle` |
| `js/ui/modals/sdc-populate-modal.js` | `SdcPopulateModal extends Modal` — live Patient search autocomplete (via `fhir-search.js`) or bare `Patient/{id}`; on Apply dispatches `AppEvents.SDC_POPULATE_REQUESTED { patientRef }`; opened from Answers ▾; `testid: sdcPopulate` |
| `js/ui/modals/def-extract-modal.js` | `DefExtractModal extends Modal` — self-wires to `AppEvents.DEF_EXTRACT_REQUESTED`; runs `definitionExtract()` over current QR, lists extracted resources, Apply downloads the Bundle via `downloadJSON`; opened from Save ▾ |
...
| `js/builder/index.js` | Builder entry point — creates `BuilderPanel()` (no-arg; self-wires via `EventState`), side-effect imports for formats/version-compat, re-exports `mount()`, `renderTree()`, `renumberAll()`, `addRootGroup()` |
| `js/builder/builder-panel.js` | `class BuilderPanel` — self-wires via `EventState.get(APP_CONTEXT_READY)` (no DI params). Owns `mount()` which wires toolbar buttons + creates `RenumberControl`. On `QUESTIONNAIRE_LOADED` runs `renderTreeAsync` + `BUILDER_EXPAND_ALL`. `_doCalcRecalc()` on `CALC_RECALC_REQUESTED`. |
...
| `js/builder/panels.js` | Action panel builders: `addPanel` (generic action-panel toggle), `buildVisPanel` (enableWhen), plus `buildQuestionSelect` helper. Former dead functions (`buildTypePanel` / `buildStylePanel` / `buildMandPanel` / `buildInitialPanel` / `buildConstraintPanel` / `buildSupportLinkPanel`) removed — those actions moved to dedicated modals |
...
| `js/nodes/item-node.js` | `ItemNode extends BaseNode` — all non-group, non-display item types; overrides `_evalCondition`, `_buildLabel`, `_buildRowContent`, `_buildConstraintBadge`, `_buildCalcBadge`, `_appendRow`; **`buildBuilder(ctx)`** — renders the builder-panel row for this item (inline answer-type row via `buildInlineTypeRow` — a full-width type dropdown + a config button `data-testid="action-type"` opening the Answer Type modal, shown in both view modes and replacing the former "Answer Type" action link; card order: title → type → id/prefix → advanced links; DnD, action links, modals via `MODAL_REGISTRY`); all builder deps injected through `ctx`; **`supportsRepeat() → true`** — overridden by `CheckboxNode` and `DisplayNode` to return `false`; `buildBuilder` uses `!node.supportsRepeat()` to hide the repeat action link; **`impliesRepeats() → false`** — overridden by `ChecklistNode` to return `true` (multi-select check-box is inherently `repeats:true`); `_buildControl` renders the "+ Add another" repeat container only when `repeats && supportsRepeat()`, so checklist/checkbox never show it |
...
| `js/nodes/choice-helpers.js` | Pure option-resolution + choiceColumn rendering helpers extracted from `choice-node.js` — `_nodeOpts`, `_evalAnswerOpts`, `_resolveColValue`, `_findRawOpt`, `_getColDisplayLabel`, `_buildColHeader`, `_buildColRow`, `_appendOptionExtras`; shared by `ChoiceNode` / `RadioNode` / `OpenChoiceNode` |
| `js/nodes/group-node.js` | `GroupNode extends BaseNode` — group nodes; overrides `_evalCondition`, `_buildLabel`, `_buildRowContent`, `_renderChildren`, `_renderDimmedChildren`, `_renderDisabledChildren`; **`refreshIcon(rc)`** — updates pass/fail icon; `_buildRowContent` applies `.lform-group-header`/`.lform-group-footer` + a badge when `_itemControl` is `header`/`footer` (questionnaire-itemControl group codes, edited via Appearance modal); **`buildBuilder(ctx)`** — renders the builder-panel row for this group (collapse toggle, child list, ⚙ gear menu — Add Group / Add Item · Copy / Paste before / Paste after · Delete — DnD, action links, style panel, modals via `MODAL_REGISTRY`); `destroy()` overrides `super.destroy()` and recursively calls `c.destroy()` on all children |
| `js/nodes/display-node.js` | `DisplayNode extends ItemNode` — `itemType:'display'`; `buildControl()` returns empty `createWrap()`; `_initRowClass` adds `.lform-item--{_displayCategory}`; `_buildLabel` renders help toggle (cat `'help'`) — toggle state stored in `this._helpOpen` (survives full re-renders) or plain label; `_buildRowContent` shows category icon (instructions/security), no interactive control; `_itemControl==='flyover'` renders an ⓘ marker whose text is revealed on hover (rich tooltip) |
...
| `js/fhir/stu3-shim.js` | STU3 → R4 normalisation shim — `normaliseSTU3(fhirJson)` returns R4-compatible copy (deep-cloned) or original unchanged if already R4; `isSTU3(fhirJson)` returns `false` immediately when the `builder-target-version` extension is present (modern R4+ file — also disambiguates the `coding` item type), otherwise detects by `meta.fhirVersion` 3.x/1.x or STU3-only item fields; converts: `item.option[]`→`item.answerOption[]`, `item.options`(Reference)→`item.answerValueSet`, `enableWhen.hasAnswer`→`operator:exists+answerBoolean`, STU3 implicit equality→`operator:'='`, `item.initial<Type>`→`item.initial:[{value<Type>}]`; works recursively; no side ef...
| `js/fhir/format-registry.js` | Unified `FormatRegistry` singleton — single source of truth for all export formats (FHIR R4/R4B/R5, REDCap CSV). API: `register(def)`, `get(id)`, `getAll()`, `getBuilderVersions()` (only `isBuilderVersion:true` entries), `detectVersion(data)` (reads the `builder-target-version` extension first, then feature heuristics: native `disabledDisplay` or `answerConstraint`→R5, else R4 — both fields are R5-only). Also exports `BUILDER_VERSION_EXTENSION_URL`, `ITEM_ANSWER_CONSTRAINT_EXTENSION_URL`, `ITEM_DISABLED_DISPLAY_EXTENSION_URL` (builder-private downgrade extension URLs) and `setBuilderVersion(q, code)` helper. Format def shape: `id`, `label`, `selectorLabel?`, `isBuilderVersion`, `metaVersion?`, `ext`, `mimeType`, `build(baseQ)`, `reportTitle?`, `validatorFilter?`, `onBeforeReport?`, `onAfterExport?`, `onCancel?` |
| `js/fhir/formats/_downgrade.js` | `backportR5ItemFields(q)` — recursively moves R5-only `Questionnaire.item` fields (`answerConstraint`, `disabledDisplay`) into builder-private extensions (`item-answerConstraint`, `item-disabledDisplay` on the github.io domain) and deletes the native fields, so R4/R4B exports are schema-valid (official HL7 cross-version extension URLs are rejected by public HAPI) while remaining loss-less on re-import |
| `js/fhir/formats/r4.js` | Self-registers R4 format into `formatRegistry` — `isBuilderVersion:true`, `metaVersion:'4.0.1'`, `selectorLabel:'FHIR R4'`, `build()` runs `backportR5ItemFields()` (R5-only fields → builder-private extensions) then stamps the `builder-target-version` extension `valueCode:'4.0.1'` |
| `js/fhir/formats/r4b.js` | Self-registers R4B format — `isBuilderVersion:true`, `metaVersion:'4.3.0'`, `selectorLabel:'FHIR R4B'`, `build()` runs `backportR5ItemFields()` (R5-only fields → builder-private extensions) then stamps the `builder-target-version` extension `valueCode:'4.3.0'` |
| `js/fhir/formats/r5.js` | Self-registers R5 format — `isBuilderVersion:true`, `metaVersion:'5.0.0'`, `selectorLabel:'FHIR R5'`, `build()` converts `open-choice→coding+answerConstraint:optionsOrString` and `choice→coding` recursively (R5 renamed the `choice` item type to `coding` and removed `open-choice`), stamps the `builder-target-version` extension `valueCode:'5.0.0'` |
...
| `js/fhir/extract.js` | `extractObservations(qr, questionnaire)` — pure SDC Observation-based extraction; returns a `transaction` Bundle of `Observation` resources; top-down `observationExtract` flag inheritance (`false` suppresses), `Quantity` from `questionnaire-unit`, multi-answer → multiple Observations |
| `js/fhir/obs-export.js` | `exportObservations(fileName, meta)` — builds Questionnaire + QR from current answers, runs `extractObservations`, downloads the Bundle; `meta`: `{ subject, author, qrId, addProfile }` |
| `js/ui/modals/obs-export-modal.js` | Observation Export modal — shown before downloading the Observations Bundle; fields: File name, Subject, Author/Performer, QR resource ID, Add SDC profile checkbox (default on); `open(suggestedName)` |
| `js/fhir/qr-builder.js` | `buildQR(fhirJson, values)` / `buildQRItem(fhirItem, values)` — builds a FHIR R4 QuestionnaireResponse; `choice`/`open-choice` → `valueCoding` (with `system`, `display`, `ordinalValue`); `quantity` → `valueQuantity {value, unit}`; `url` → `valueUri`; `reference` → `valueReference`; exported `buildAnswer(fhirItem, v)` builds a single typed answer object (reused by calc.js to write computed values back into the QR); used by qr-export and FHIRPath calculatedExpression evaluation |
| `js/fhir/calc.js` | FHIRPath calculatedExpression / initialExpression evaluation. `buildVarEnv(variables, qr, fp)` resolves SDC `%variables`; `evalCalcNodes(nodes, qr, fp, values, envVars, base?)` evaluates readOnly `calculatedExpression` nodes **in topological dependency order** (via `dep-graph.js`) and, when `base` is supplied, writes each computed value back into the live QR so calc chains (A→B→C) resolve in one pass; `evalInitialExprNodes(...)` evaluates `initialExpression` |
| `js/fhir/dep-graph.js` | Pure dependency-graph module (no DOM/state). `extractRefs(expr)` parses `linkId='x'` and `%var` references from FHIRPath; `buildDepGraph(nodes, variables)` → `{edges, nodeIds, varNames, missing}` (edge A→B means A depends on B, sourced from `enableWhen[].question`, `enableWhenExpression`, `_calculatedExpr`, `_initialExpr`); `detectCycles(graph)` → array of cyclic paths (DFS coloring); `topoOrder(graph)` → `{order, cycles}` (Kahn's algorithm, cyclic nodes appended in tree order) |
...
| `js/ui/modals/metadata-modal.js` | Questionnaire Properties modal — `open(questMeta)`; draft pattern; renders all fields via `renderMetaSections(container, pending, questMeta)`; `MetadataCard` dispatches `METADATA_EDIT_REQUESTED` with `{ questMeta }` in detail; listener calls `_modal.open(questMeta)` |
...
| `js/ui/modals/modal-base.js` | `Modal` base class — `constructor({cancelLabel, applyLabel, maxWidth, bodyClass})` builds and appends modal DOM to `document.body`; exposes `this.body`, `this.title`; `open()` shows backdrop, `close()` hides it; subclass overrides `open()`, `_apply()`, `_cancel()`; single shared Escape handler closes topmost open modal. Modals are stateless — `open()` receives all needed data as params; callers pass data from `EventState.get(APP_CONTEXT_READY)` at call time. Never monkey-patch `_modal._apply = fn` — always use `extends Modal`. |
...
| `js/ui/modals/states-sections/observation-extract.js` | `ObservationExtractSection` — "Extract as Observation" checkbox (`sdc-questionnaire-observationExtract`); `commit()` writes `node._observationExtract`; hidden for `display` items |
...
| `js/ui/modals/states-sections/is-subject.js` | `IsSubjectSection` — "Is subject" checkbox (`sdc-questionnaire-isSubject`); `commit()` writes `node._isSubject`; visible for non-`display` item nodes only; marks the item whose answer is the QR subject |
| `js/ui/modals/answer-type/modal.js` | Answer Type edit modal — `open(node, typeLink, setActive, questDoc, answerStore)`; draft pattern; sections receive `build(pending, questDoc, answerStore)` and `commit(pending, node, questDoc, answerStore)`; item-node.js reads `questDoc`/`answerStore` from `EventState` and passes to `open()` |
...
| `js/ui/answer-type/sections/choice.js` | `ChoiceSection` — choice/radio/open-choice: answer-source toggle (options list / ValueSet / answerExpression / candidateExpression), openLabel; uses `createOptionsEditor` (with `showType` when `_rawAnswerOptions` present) for the unified Code+Label+Score+Prefix+Type row editor; `commit()` writes `_answerValueSet`, `options`, `_rawAnswerOptions`, `_optionOrdinals`, `_optionPrefixes`, `_openLabel`, `_answerExpression`, `_candidateExpression` (answerExpression / candidateExpression are mutually exclusive, each clears the other); uses `fhirOptsToStr` from `import-helpers.js` to sync `node.options` after raw rebuild |
...
| `js/ui/modals/answer-type/sections/column-count.js` | `ColumnCountSection` — `sdc-questionnaire-columnCount` dropdown (radio / checklist); lays out options across N columns in preview; `commit()` writes `_columnCount` |
...
| `js/ui/node-gear-menu.js` | `NodeGearMenu` — ⚙ gear dropdown in the top-right of a builder node (replaces the old `×` delete button and the `⊕ Add ▾` menu). `addItem(label, testid, onClick, {destructive, disabled})` + `addSep()`; follows the `CLOSE_DROPDOWNS` toggle pattern; adds `node-gear-wrap--open` to elevate its stacking context while open (so it renders above sibling nodes' gear buttons). Groups get **Add Group / Add Item · Copy / Paste before / Paste after · Delete** (gear button keeps `data-testid="group-add-btn"`); items get **Copy / Paste before / Paste after · Delete** (`data-testid="node-gear-btn"`). Paste items are disabled (`node-gear-menu-item--disabled`) until the clipboard has content, wired via `addCopyPasteGearItems` in `builder-helpers.js`. The gear is not `advanced-ctrl`, so it stays visible in Simple mode |
...
| `js/ui/left-header-actions.js` | Mounts `MoreMenu` into `[data-mount="left-header-right"]` in the builder header; self-mounts on import |
| `js/ui/menus/more-menu.js` | `MoreMenu extends DropdownMenu` — `⋯` button in builder header; contains **Simple / Advanced** view-mode radio items (dispatch/reflect `BUILDER_VIEW_MODE_CHANGE`, checkmark on active), **Expand all / Collapse all** tree controls (dispatch `BUILDER_EXPAND_ALL` / `BUILDER_COLLAPSE_ALL`; moved here from the tree toolbar), Undo, Redo (with `data-mount` for `UndoRedo`), separator, FHIR Reference (opens `HelpModal`), Settings (opens `settings.html`) |
| `js/ui/simple-mode.js` | `SimpleMode` — toggles the `builder--simple` class on `[data-mount="left-panel"]`, hiding advanced per-item action links (`[data-role="advanced-ctrl"]`, set by `makeActionLink`) while keeping the Add ▾ menu, delete and structure. Persists `builderViewMode` in `localStorage` (default `advanced`). Pure CSS layer — never re-renders on mode change. Event-driven single source of truth via `BUILDER_VIEW_MODE_CHANGE`; created in `app.js` |
| `js/nodes/change-type.js` | Shared answer-type-change core used by both the Answer Type modal and the inline type selector (single source of truth). `changeNodeType(node, newType, tree, answerStore)` — clears stored answers on real type change, creates a new node of the target class via the leaf `NODE_REGISTRY` (avoids a circular `index.js` import), splices it into the tree, normalises `repeats`, returns the new node. `nodeTypeNeedsConfig(node)` — true for any choice-family item (options are defined in the Answer Type dialog), used to keep the inline config button highlighted so it's clear the setup lives in the modal |
...
| `js/ui/menus/answers-menu.js` | `AnswersMenu extends DropdownMenu` — Load QR file / Load from Library / ↧ Fill from FHIR Server… (opens `SdcPopulateModal`); hidden file input (`data-testid="qr-file-input"`); initially hidden, shown when tree has nodes; fully event-driven |
| `js/ui/menus/save-menu.js` | `SaveMenu extends DropdownMenu` — Cloud Save / Questionnaire… / QR JSON; `this._questDoc`/`this._answerStore` populated via `APP_CONTEXT_READY`+`QUESTIONNAIRE_LOADED` events (instance fields); `promptExport()` delegates to `saveFormatModal`; Cloud Save click dispatches `CLOUD_SAVE_REQUESTED` event |
...
| `js/ui/menus/settings-menu.js` | `SettingsMenu extends DropdownMenu` — Preferences toggles (Tips, Autosave, Local validation, Server validation) + tool actions (Validate, Expand all, Collapse all); fully event-driven — listens to `TIPS_INIT_DONE`, `AUTOSAVE_INIT_DONE`, `AUTOSAVE_SAVED`; validator toggles dispatch `AppEvents.VALIDATOR_TOGGLE {id, enabled}`; right-aligned dropdown; hidden until tree has nodes |
...
| `js/fhir/validators/external.js` | `ExternalValidator extends Validator` — `id='external'`; `enabled=false` by default; POSTs to a FHIR server `$validate` endpoint via CORS proxy (`corsProxyUrl` from config.json); **routes by target FHIR version** — swaps the configured `/baseRxx` segment for `baseR4`/`baseR4B`/`baseR5` based on injected `getFhirTarget()` (falls back to the configured URL if it has no recognised base segment); `get name()` appends the active version (e.g. "HAPI FHIR R5") so the report section reflects the validated version; **strips the builder-target-version extension** from the validated payload (server-internal marker; exported file keeps it); retries configurable on network/5xx errors only; non-retryable 4xx (incl. HTTP 413 Payload Too Large → clear "questionnaire too large" message) raised as `ValidatorFatalError`; maps OperationOutcome issues to `{ severity, nodeId: '(external)', message }` |
...
| `js/ui/term-search.js` | `TermSearch` class — LOINC / SNOMED CT / ICD-10-CM autocomplete widget; `new TermSearch(container, { onSelect })` mounts into any element; first-use shows ToU acceptance banner (stored in `localStorage`); system selector (LOINC / SNOMED CT / ICD-10-CM) via `createCustomSelect`; LOINC + ICD-10 use NLM Clinical Tables API directly (no proxy); SNOMED CT uses `terminologyService.expandWithFilter()` → FHIR `$expand` via CORS proxy; per-system `minChars` (2 for LOINC/ICD-10, 4 for SNOMED); `onSelect({ system, code, display })` called on pick; `destroy()` for cleanup |
...
| `css/sdc-modals.css` | Styles for `SdcPopulateModal` (`.sdc-pop-*` — patient search field/label/input, `.sdc-pop-input--error`, `.sdc-pop-drop` fixed-position autocomplete above modal) and `DefExtractModal` (`.def-extract-*` — warn/summary/list/card/type/id/fields) |
...
| `js/fhir/validate.js` | `validateTree(tree, values, questMeta?)` → `{severity,nodeId,message}[]`; enforces all FHIR R4 invariants que-0–que-13 (error or warning per rule); FHIRPath syntax check for `_calculatedExpr`, `_answerExpression`, `_candidateExpression`, `enableWhenExpression`, constraint expressions; empty titles, missing options; **circular dependency detection** (via `dep-graph.js` `detectCycles`) — errors on any loop across enableWhen/expression references; cross-field semantic warnings: `required+hidden`, `calculatedExpression` without `readOnly`, `answerExpression+answerOption[]`, `candidateExpression+answerOption[]`, `enableWhen+enableWhenExpression`, `repeats` mismatch with `_initialValues`, sliderStep decimal, displayCategory on display item |
| `js/fhir/server-config.js` | Provider-based config system. Exports `serverConfig` singleton + `ServerConfigProvider` base, `DefaultConfigProvider` (config.json), `LocalStorageConfigProvider` (`fhirqb.server.*`), `SupabaseConfigProvider` (cloud, registered by `server-config-cloud.js`). `serverConfig.register(provider)` — last wins; `get(key)` / `set(key, val)` / `reset(key)` / `load(url)` / `ready()`. Keys: `CONFIG_KEYS` (`corsProxyUrl`, `terminologyServer`, `nlmApiBaseUrl`, `fhirBaseUrl`, `validators`). `_clear()` for test cleanup. |
| `js/fhir/server-config-cloud.js` | Supabase cloud sync for server settings. Registers `SupabaseConfigProvider` at highest priority. `onAuthChange` → `loadSettings(userId)` hydrates provider from `user_settings` table; logout → `clearSettings()`. `saveSettings(userId)` upserts to Supabase. Supabase table required: `user_settings (id, user_id FK, server_config jsonb, updated_at)` with RLS `auth.uid() = user_id`. |
| `js/fhir/terminology-service.js` | Singleton `terminologyService` — reads `corsProxyUrl`/`terminologyServer`/`nlmApiBaseUrl` via `serverConfig` (no own fetch); `getServer(node, questMeta)` (fallback chain: per-item → questionnaire → default); `expandValueSet`/`expandWithFilter`/`testServer`/`testExpand`/`expandAll` |
...
| `js/ui/modals/save-format-modal.js` | `SaveFormatModal extends Modal` — opened by `SaveMenu` on "Questionnaire…" click; format dropdown built from `formatRegistry.getAll()` (FHIR R4/R4B/R5 JSON + REDCap CSV); `open(deps)` receives `deps.fhirTarget` from caller (save-menu reads from EventState); generic `_exportWithFormat(fmt)`: calls `buildFHIRObject()`, invokes `fmt.onBeforeReport?.()`, shows `validateModal` with `fmt.validatorFilter` (external FHIR server validator suppressed for REDCap), on confirm calls `fmt.build(baseQ)` + downloads via `downloadJSON` (JSON) or Blob (CSV); cleanup hooks `fmt.onAfterExport/onCancel` wired; remembers last format in `localStorage` (`fhirqb-save-format`) |
...
| `sampledata/r4b-demo.fhir.json` | **FHIR R4B Demo** — `builder-target-version` extension `4.3.0`; demonstrates `answerConstraint`, R4B-native `disabledDisplay`, auto-detected and loaded as R4B on import |
| `sampledata/r5-demo.fhir.json` | **FHIR R5 Demo** — `builder-target-version` extension `5.0.0`; demonstrates `answerConstraint` and `disabledDisplay` as native R5 fields; `open-choice` items exported with R5 `choice+answerConstraint` |
...
| `sampledata/answer-expression-demo.fhir.json` | **Answer Expression Demo** — PHQ-2 questionnaire demonstrating SDC answer-source expressions: `sdc-questionnaire-answerExpression` (Severity Level, Recommended Action) and `sdc-questionnaire-candidateExpression` (Suggested follow-up option), both computing choice options at render time from a `%phq2score` variable via FHIRPath |
...
| `sampledata/reference-example.fhir.json` | **Care Referral Request** — `reference` item type (Patient, Practitioner, Encounter) with `questionnaire-referenceResource`; `sdc-questionnaire-isSubject` on the Patient reference (SUBJECT badge in preview); urgency choice with SNOMED code; reason/history text items; `rendering-style` on group headers; `version`/`publisher`; a `sdc-questionnaire-definitionExtract` group (Save ▾ → Definition Extract) |
| `sampledata/observation-extract-demo.fhir.json` | **Observation Extraction Demo** — SDC `sdc-questionnaire-observationExtract` on a Vital Signs group; coded `decimal` items with `questionnaire-unit` (weight/height/temperature → `Quantity`), coded `choice` (smoking → `CodeableConcept`), a `false`-suppressed coded item, and a codeless item; demonstrates **Save → Observations** export |
...
| `sampledata/item-control-demo.fhir.json` | **Item Control Demo** — `questionnaire-itemControl` codes: `check-box` (checklist, with `sdc-questionnaire-columnCount: 2` for a 2-column option layout), `autocomplete` (searchable dropdown), `text-area` (multi-line string), `spinner` (integer), `drop-down` (explicit default), `slider` (integer range slider), `lookup` (choice live-search against FHIR terminology server); also demos `questionnaire-usageMode` (display-only item), `sdc-questionnaire-itemMedia` (image on choice), `sdc-questionnaire-itemWeight` (weighted scoring on options) |
...
| `tests/e2e/simple-mode.spec.js` | E2E tests (6) — Simple/Advanced view mode in the More ▾ menu: advanced is default (action links visible), Simple hides `[data-role="advanced-ctrl"]` links but keeps Add ▾, switching back re-shows them, active mode checkmark, persistence across reload, and Expand all / Collapse all tree controls |
| `tests/e2e/node-gear-menu.spec.js` | E2E tests (2) — ⚙ node gear menu: group gear exposes Add Group / Add Item / Delete; item gear exposes only Delete (no add options) |
...
| `tests/e2e/is-subject.spec.js` | E2E tests (7) — `sdc-questionnaire-isSubject`: SUBJECT badge visible in builder preview for the subject item (absent for others), States modal "Is subject" checkbox reflects/toggles the flag (check adds badge, uncheck removes it), export round-trip emits the extension on the subject item only; fixture `tests/fixtures/is-subject.fhir.json` |
| `tests/e2e/column-count.spec.js` | E2E tests (7) — `sdc-questionnaire-columnCount`: radio option list gets `ctrl-wrap--columns` + `--col-count` in preview (absent for plain item), Answer Type modal "Option columns" select reflects imported value / default, setting a value applies the columns layout and clearing removes it, export round-trip emits the extension on the columned item only; fixture `tests/fixtures/column-count.fhir.json` |
...
| `tests/calc.test.js` | Unit tests for `js/fhir/calc.js` — `buildVarEnv`, `evalCalcNodes` incl. topological calc-chain resolution |
| `tests/dep-graph.test.js` | Unit tests for `js/fhir/dep-graph.js` — `extractRefs`, `buildDepGraph`, `detectCycles`, `topoOrder` (21 tests) |
| `tests/validate.test.js` | Unit tests for `js/fhir/validate.js` — `validateTree`, linkId uniqueness, FHIRPath syntax (calculatedExpr + answerExpression + candidateExpression), constraint key checks, fhirpath-unavailable path, all cross-field semantic rules, all R4 invariants que-0–que-13 |
...
| `tests/import-extensions.test.js` | Unit tests for `js/fhir/import.js` — extension suites: group items, unknown extensions, replaces, collapsible, openLabel, designNote, answerExpression, candidateExpression, regex, optionExclusives (40 tests) |
