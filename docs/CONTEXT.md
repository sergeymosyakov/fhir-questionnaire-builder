...
| `js/ui/answer-type/base-section.js` | `AnswerTypeSection` base class + `SECTION_REGISTRY` array; base declares four methods: `isVisible(type)`, `build(pending)→HTMLElement`, `onTypeChange(type)`, `commit(pending, node)` — all no-ops; subclasses override all four |
...
| `js/ui/answer-type/data.js` | Pure constants and helpers: `CHOICE_TYPES`, `ENTRY_FORMAT_TYPES`, `NUMERIC_TYPES`, `ITEM_TYPES`, `FHIR_R4_TYPES`, `BUILDER_UNITS`, `_optsWithOrdinals(node)`, `_parseOptsWithOrdinals(str)` |
