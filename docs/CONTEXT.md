...
| `js/fhir/validators/init.js` | Reads `config.json` and registers validators into `validatorRegistry`; accepts `{ localEnabled, externalEnabled }` from prefs so validators start in correct state; called once at startup from `app.js` |
