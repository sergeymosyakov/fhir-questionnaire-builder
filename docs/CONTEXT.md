...
| `js/app.js` | Entry point — wires all modules at startup; registers `LocalStorageAdapter`; wires undo/redo buttons + Ctrl+Z/Y/Shift+Z keyboard shortcuts; creates `PreviewForm`, `QuestionnaireLoader`; calls `configure()` on all services; listens for `CLOUD_SAVE_REQUESTED` / `CLOUD_LOAD_REQUESTED` events dispatched by menus |
...
| `js/preview-form.js` | Right panel — `PreviewForm` class; reactive preview driven by `effect()`; `reinitForm({ silent? })` rebuilds form (progress bar suppressed when `silent:true` — used for background ValueSet expansion re-evals); `BUILDER_NAVIGATE` handler scrolls preview without touching `_formTick`; exports `getLastCtx()` compat shim |
...
| `js/nodes/display-node.js` | `DisplayNode extends ItemNode` — `itemType:'display'`; `buildControl()` returns empty `createWrap()`; `_initRowClass` adds `.lform-item--{_displayCategory}`; `_buildLabel` renders help toggle (cat `'help'`) — toggle state stored in `this._helpOpen` (survives reactive re-renders) or plain label; `_buildRowContent` shows category icon (instructions/security), no interactive control |
...
| `js/events.js` | `AppEvents` — central registry of all custom event name constants: `QUESTIONNAIRE_LOADED`, `QUESTIONNAIRE_CLEARED`, `BUILDER_RERENDER`, `BUILDER_NAVIGATE`, `PREVIEW_NAVIGATE_TO`, `REINIT_FORM`, `SHOW_JSON`, `VIEW_PREF_CHANGE`, `PREVIEW_MODE_CHANGE`, `PATIENT_CTX_APPLIED`, `QR_LOADED`, `RENUMBER_PROGRESS`, `RENUMBER_DONE`, `QUESTIONNAIRE_RESET`, `CLOUD_SAVE_REQUESTED`, `CLOUD_LOAD_REQUESTED` |
...
| `js/fhir/questionnaire-loader.js` | `QuestionnaireLoader` — import pipeline; constructor receives injected `{ tree, values, questMeta, reinitForm }`; `load(data, fileName)` runs FHIR import + validate + render + `await reinitForm()`; `_expandValueSets()` calls `reinitForm({ silent: true })` to suppress progress bar |
...
| `sampledata/item-control-demo.fhir.json` | **Item Control Demo** — `questionnaire-itemControl` codes: `check-box` (checklist), `autocomplete` (searchable dropdown), `text-area` (multi-line string), `spinner` (integer), `drop-down` (explicit default) |
...
- **Playwright** — E2E test suite; **572 tests** across 45 spec files (Chromium); CI via GitHub Actions (`npx playwright test`)
...
- **Vitest** — unit test suite for pure-function modules; **774 tests** across 14 files; CDN imports mocked via `vi.mock`; CI via GitHub Actions (`npm test`)
...
- `type:group` → group node; `type:boolean` → `itemType:'checkbox'`; `type:choice` → `itemType:'select'` or `'radio'` (if `questionnaire-itemControl: radio-button`) or `'checklist'` (if `check-box`); `autocomplete`/`drop-down`/`text-area`/`text-box`/`spinner` → stored as `node._itemControl`
...
- `node._itemControl` → exports corresponding `questionnaire-itemControl` extension code (`autocomplete`, `drop-down`, `text-area`, `text-box`, `spinner`)
