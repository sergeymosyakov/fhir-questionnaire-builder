...
| `http://hl7.org/fhir/StructureDefinition/questionnaire-displayCategory` | standard | `_displayCategory` (`instructions` / `security` / `help`; applies visual category styling to `display` items in preview; editable in Answer Type modal); exported with `system: 'http://hl7.org/fhir/questionnaire-display-category'` in coding | Yes |
...
| `questionnaire-displayCategory` on `display` items | `http://hl7.org/fhir/StructureDefinition/questionnaire-displayCategory` | R4 (group items only), R5 (any item) | R4 context invariant limits it to group items; R5 allows it on display items |
| `questionnaire-sliderStepValue` with decimal value | `http://hl7.org/fhir/StructureDefinition/questionnaire-sliderStepValue` | R4 (integer only), R4B/R5 (decimal) | R4 extension definition specifies `valueInteger`; decimal slider steps are R4B/R5 |
...
|---|---|---|
| `"A code with no system has no defined meaning"` | Our choice-type answer options store `code`+`display` without a `system`; `system` is optional in R4 `Coding` | Accept — `system` is optional in FHIR `Coding`; builder UI does not have a system field for simple option lists |
...
| `dom-6` (resource should have narrative) | HAPI warns about missing `text.div` (XHTML narrative) | Accept — narrative is optional and not generated by this builder |
