...
// Group
{
  ...
  children:            Node[],
  // в”Ђв”Ђ also possible on groups (imported/exported; editable via Props button) в”Ђв”Ђ
...
// Item
{
  ...
  repeats:             boolean,          // FHIR item.repeats вЂ” multi-row input in preview
  ...
  _initialExpr:        string,           // FHIRPath expression (SDC initialExpression) вЂ” evaluated once on import + Re-init
  ...
  _answerValueSet:     string,           // FHIR item.answerValueSet URL вЂ” preserved round-trip; not resolved to options
  ...
  _optionOrdinals:     object,           // map of option code в†’ ordinalValue (from ordinalValue extension on answerOption.extension or valueCoding.extension fallback)
  _sliderStep:         number,           // questionnaire-sliderStepValue ext; when set, integer/decimal renders as <input type="range"> slider
  _disabledDisplay:    string,           // 'hidden'|'protected' вЂ” behaviour when enableWhen condition is not met; 'protected' is default (not persisted)
  _supportLinks:       string[],         // questionnaire-supportLink URIs (0..*); рџ”— icons in builder preview; "More info в†—" buttons in patient view
...
|---|---|---|---|
| `questMeta.id` | `Questionnaire.id` | в†ђ `id` | в†’ `id` (fallback: `'logic-builder-export'`) |
| `questMeta.url` | `Questionnaire.url` | в†ђ `url` | в†’ `url` (omitted when empty) |
| `questMeta.version` | `Questionnaire.version` | в†ђ `version` | в†’ `version` (omitted when empty) |
| `questMeta.name` | `Questionnaire.name` | в†ђ `name` | в†’ `name` (omitted when empty) |
| `questMeta.title` | `Questionnaire.title` | в†ђ `title` | в†’ `title` (takes precedence over rawFhir.title; fallback: `'Untitled Questionnaire'`) |
| `questMeta.status` | `Questionnaire.status` | в†ђ `status` (default: `'draft'`) | в†’ `status` |
| `questMeta.date` | `Questionnaire.date` | в†ђ `date` | в†’ `date` (preserved from import; falls back to today's ISO date for new questionnaires) |
| `questMeta.publisher` | `Questionnaire.publisher` | в†ђ `publisher` | в†’ `publisher` (omitted when empty) |
| `questMeta.description` | `Questionnaire.description` | в†ђ `description` | в†’ `description` (omitted when empty) |
| `questMeta.purpose` | `Questionnaire.purpose` | в†ђ `purpose` | в†’ `purpose` (omitted when empty) |
| `questMeta.copyright` | `Questionnaire.copyright` | в†ђ `copyright` | в†’ `copyright` (omitted when empty) |
| `questMeta.approvalDate` | `Questionnaire.approvalDate` | в†ђ `approvalDate` | в†’ `approvalDate` (omitted when empty) |
| `questMeta.lastReviewDate` | `Questionnaire.lastReviewDate` | в†ђ `lastReviewDate` | в†’ `lastReviewDate` (omitted when empty) |
| `questMeta.subjectType` | `Questionnaire.subjectType` | в†ђ `subjectType[]` joined as comma-separated string (default: `'Patient'`) | в†’ split back to array (default: `['Patient']`) |
| `questMeta.effectivePeriodStart` | `Questionnaire.effectivePeriod.start` | в†ђ `effectivePeriod?.start` (default: `''`) | в†’ `effectivePeriod.start` (omitted when empty) |
| `questMeta.effectivePeriodEnd` | `Questionnaire.effectivePeriod.end` | в†ђ `effectivePeriod?.end` (default: `''`) | в†’ `effectivePeriod.end` (omitted when empty) |
| `questMeta.experimental` | `Questionnaire.experimental` | в†ђ `experimental` (default: `null`) | в†’ `experimental` (omitted when `null`) |
| `questMeta.language` | `Questionnaire.language` | в†ђ `language` (default: `''`) | в†’ `language` (omitted when empty) |
| `questMeta._rawIdentifier` | `Questionnaire.identifier[]` | в†ђ stored as Identifier[] (default: `[]`) | в†’ written back unchanged; editable via **Identifiers** collapsible section in Properties modal (use/system/value rows; badge shows count) |
| `questMeta._rawContact` | `Questionnaire.contact[]` | в†ђ stored as-is (pass-through) | в†’ written back unchanged (omitted when null) |
| `questMeta._rawUseContext` | `Questionnaire.useContext[]` | в†ђ stored as-is (pass-through) | в†’ written back unchanged (omitted when null) |
| `questMeta._rawJurisdiction` | `Questionnaire.jurisdiction[]` | в†ђ stored as-is (pass-through) | в†’ written back unchanged (omitted when null) |
| `questMeta._rawCode` | `Questionnaire.code[]` | в†ђ stored as array (default: `null`) | в†’ written back unchanged; editable via **Codes** section in Properties modal (system/code/display rows; draft pattern; badge shows count) |
| `questMeta.derivedFrom` | `Questionnaire.derivedFrom[]` | в†ђ stored as string array (default: `[]`) | в†’ written back as array; editable via **Derived From** collapsible section in Properties modal; round-trip safe |
| `questMeta._metaVersionId` | `Questionnaire.meta.versionId` | в†ђ `meta.versionId` (default: `''`) | в†’ written back when set; editable in Properties modal вЂ” **Resource Meta** section; **Generate** button creates a fresh UUID v4 |
| `questMeta._metaSource` | `Questionnaire.meta.source` | в†ђ `meta.source` (default: `''`) | в†’ written back when set; editable URI input in Properties modal вЂ” Resource Meta section |
| `questMeta._metaLastUpdated` | `Questionnaire.meta.lastUpdated` | в†ђ `meta.lastUpdated` displayed read-only in Properties modal | в†’ **always** replaced with `new Date().toISOString()` on every export |
| `questMeta._rawMetaProfile` | `Questionnaire.meta.profile[]` | в†ђ stored as string array (default: `[]`) | в†’ written back as array; editable list of canonical URLs in Properties modal вЂ” Resource Meta section |
| `questMeta._rawMetaTag` | `Questionnaire.meta.tag[]` | в†ђ stored as Coding[] (default: `[]`) | в†’ written back unchanged; editable system/code/display rows in Properties modal вЂ” Resource Meta section |
| `questMeta._rawMetaSecurity` | `Questionnaire.meta.security[]` | в†ђ stored as Coding[] (default: `[]`) | в†’ written back unchanged; editable system/code/display rows in Properties modal вЂ” Resource Meta section |
| `questMeta._rawText` | `Questionnaire.text` | в†ђ stored as Narrative `{ status, div }` when present (default: `null`) | в†’ if preserved: written back unchanged; if null: auto-generated from title/status/items (`status: "generated"`, XHTML div via `generateNarrativeDiv()`) вЂ” always present in export |
...
| `choice` | `select` | unless `questionnaire-itemControl: radio-button` в†’ `radio` |
...
| `date` | `date` | |
| `dateTime` | `dateTime` | Stored as `YYYY-MM-DDTHH:MM:SS`; QR в†’ `valueDateTime` |
| `time` | `time` | Stored as `HH:MM:SS`; QR в†’ `valueTime` |
| `url` | `url` | format validated with `new URL()` |
| `attachment` | `attachment` | file input, stores `{name, size, type}` |
| `group` | `group` (node.type) | |

### Export: `itemType` в†’ `item.type`
...
| `number` | `decimal` | legacy alias вЂ” kept for backward compatibility with saved questionnaires |
| `text` | `string` | |
...
|---|---|---|---|
| `id` | `item.linkId` | в†ђ `linkId` | в†’ `linkId` |
| `title` | `item.text` | в†ђ `text` | в†’ `text` |
| `mandatory: true` | `item.required: true` | в†ђ `required` | в†’ `required: true` |
| `mandatory: false` | `item.required: false` | в†ђ `required` | в†’ `required: false` |
| `mandatory: null` | *(omitted)* | `required` absent в†’ `null` | not written |
...
| `_supportLinks` | `questionnaire-supportLink` ext (0..*) | help / documentation URIs; rendered as рџ”— icons in builder preview and "More info в†—" buttons in patient view; editable via **Props** button |
...
|---|---|---|
| `options` | `item.answerOption[]` | comma-split в†’ `valueCoding.{code, display}` on export; reverse on import |
...
| `_codes` | `item.code[]` | imported and exported unchanged (round-trip safe); editable via **Props** button (codes-modal вЂ” system/code/display rows, draft pattern); also supported on groups (see Group-specific) |
| `_maxLength` | `item.maxLength` | imported в†’ `node._maxLength`; exported back when set; character counter + `maxlength` attribute enforced in preview |
...
| `_optionOrdinals` | `ordinalValue` ext on `answerOption[].extension` (primary) or `valueCoding.extension` (fallback) | map of option code в†’ numeric score; shown as `(N)` badge in radio/select; editable in Answer Type modal (`code=Label=score` format); exported to `answerOption.extension` |
...
| `_disabledDisplay` | `item.disabledDisplay` (R4B native field) + R4 backport extension `extension-Questionnaire.item.disabledDisplay` | `'hidden'` в†’ item removed from DOM when not visible; `'protected'` (default) в†’ grayed row; editable in Show When modal |
| `_minOccurs` | `questionnaire-minOccurs` ext (`valueInteger`) | imported/exported when `node.repeats === true` |
| `_maxOccurs` | `questionnaire-maxOccurs` ext (`valueInteger`) | imported/exported when `node.repeats === true`; enforced in preview вЂ” add button disabled at limit |
| `_answerValueSet` | `item.answerValueSet` | imported в†’ `node._answerValueSet`; exported back unchanged; URL not resolved вЂ” items show no selectable options in the builder |
| `_initialValue` | `item.initial[0]` value | imported from `initial[0]`; exported as `initial: [entry]`; pre-fills `values[]` on import |
| `_initialValues` | `item.initial[]` all values | set only for repeating items with >1 initial value; exported as `initial: [entry, вЂ¦]`; `_initialValue` holds `initial[0]` for backwards compat |
...
1. Standard `item.enableWhen[]` в†’ `node.enableWhen[]` (copied as-is)
2. `item.enableBehavior` в†’ `node.enableBehavior` (`'all'` | `'any'`; default `'all'`)
3. SDC `sdc-questionnaire-enableWhenExpression` extension в†’ `node.enableWhenExpression` (FHIRPath string)
4. `enableWhen` also generates `_enableWhenText` (human-readable label, e.g. `В«QВ» = Yes AND В«Q2В» = No`)

### Export

- `node.enableWhen[]` в†’ `item.enableWhen[]` (shallow copy)
- `node.enableBehavior === 'any'` в†’ `item.enableBehavior: 'any'` (omitted otherwise)
- `node.enableWhenExpression` в†’ SDC `sdc-questionnaire-enableWhenExpression` extension
...
| `http://hl7.org/fhir/StructureDefinition/questionnaire-supportLink` | standard | `_supportLinks` (0..* help/documentation URIs per item or group; рџ”— icons in builder; "More info в†—" in patient view; editable via **Props** button) | Yes |
...
|---|---|---|---|
| `status` | в†ђ preserved in `qrMeta.status` | в†’ written; editable in QR Export modal | Default: `'in-progress'` |
| `subject` | в†ђ `subject.reference` in `qrMeta.subject` | в†’ `subject.reference` when non-empty | Optional; editable in QR Export modal |
| `author` | в†ђ `author.reference` in `qrMeta.author` | в†’ `author.reference` when non-empty | Optional; editable in QR Export modal |
| `authored` | not stored | в†’ `new Date().toISOString()` | Always set to current time on export |
...
**Minor gaps:** `id` is not preserved on import (generated fresh on export). `implicitRules`, `language`, and `meta` are not preserved вЂ” pass-through not implemented for QR-level meta.

---

## Not Supported / Partial Support

Legend: вљ пёЏ = silent data loss (field present in import file, ignored or overwritten on export); вќЊ = not handled at all; рџ”§ = partial support.

### Questionnaire root-level вЂ” silent data loss on export
...
|---|---|---|
| `Questionnaire.implicitRules` | вљ пёЏ Silently dropped | Declares the rules set that constrains how the resource is used. Rare in practice. |
| `Questionnaire.implicitRules` | вљ пёЏ Silently dropped | Declares the rules set that constrains how the resource is used. Rare in practice. |
| Unknown item extensions | вљ пёЏ Silently dropped | Any `item.extension[]` entry whose URL is not explicitly handled by the builder is discarded on import and will not appear in the exported JSON. |

### Item-level вЂ” not implemented

| FHIR field / extension | Status | Notes |
|---|---|---|
| `answerConstraint` | вќЊ Not handled | R4B/R5 field (`optionsOnly` / `optionsOrType` / `optionsOrString`) |
| `item.answerValueSet` вЂ” external URL | рџ”§ URL preserved round-trip | Not resolved to answer options; no FHIR terminology server integration. `#id` contained refs ARE resolved (see Round-Trip Safety) |
| `Questionnaire.contained[]` | рџ”§ Preserved round-trip | Viewable as JSON in the Contained card; not otherwise editable |
| Resource reference resolution | рџ”§ Partial | `type: 'reference'`: resource-type dropdown + id text input; no live FHIR server search |
| `minLength` | вќЊ Not handled | Minimum allowed response length for `string` / `text` / `url` items. Counterpart to `maxLength` which is fully supported. |
| `maxDecimalPlaces` | вќЊ Not handled | Maximum number of decimal places for `decimal` items (`http://hl7.org/fhir/StructureDefinition/maxDecimalPlaces`). |
| `regex` | вќЊ Not handled | Regular expression validation pattern for `string` / `text` / `url` items (`http://hl7.org/fhir/StructureDefinition/regex`). |
| `mimeType` | вќЊ Not handled | One or more allowed MIME types for `attachment` items (`http://hl7.org/fhir/StructureDefinition/mimeType`). Multiple values permitted. |
| `maxSize` | вќЊ Not handled | Maximum file size in MB for `attachment` items (`http://hl7.org/fhir/StructureDefinition/maxSize`). |
| `questionnaire-optionPrefix` | вќЊ Not handled | Display prefix for each `answerOption` (e.g. `"A."`, `"1."`). Commonly used in clinical forms with lettered or numbered options. Silently dropped on import. |
| `questionnaire-unitValueSet` | вќЊ Not handled | ValueSet of selectable units for `quantity` items (alternative to `questionnaire-unitOption` / `sdc-questionnaire-unitOption`). |
| `questionnaire-usageMode` | вќЊ Not handled | Controls when the item is relevant: `capture` / `display` / `display-non-empty` / `capture-display` / `capture-display-non-empty`. |
| `questionnaire-referenceFilter` | вќЊ Not handled | FHIRPath expression used to filter valid reference targets for `reference` items. |
| `questionnaire-referenceProfile` | вќЊ Not handled | Profile URL that restricts valid resource types for `reference` items (complementary to `questionnaire-referenceResource` which is supported). |
| `questionnaire-signatureRequired` | вќЊ Not handled | Indicates that a digital signature is required for the item or group. |
| `questionnaire-baseType` / `questionnaire-fhirType` | вќЊ Not handled | Base FHIR type for items derived from `ElementDefinition` (used with `item.definition`). |
| `designNote` | вќЊ Not handled | Author-facing design note text; not intended to be displayed to end users. Silently dropped. |
...
### SDC extensions вЂ” not implemented (no server required)

| Extension | Status | Notes |
|---|---|---|
| `sdc-questionnaire-answerExpression` | вќЊ | Dynamic answer options derived from FHIRPath over form values (no server needed) |
| `sdc-questionnaire-itemWeight` | вќЊ | Per-option weight for scoring (analogous to `ordinalValue` at item level) |
| `sdc-questionnaire-unitOption[]` | вќЊ | Multiple selectable units for `quantity` items |
| `sdc-questionnaire-shortText` | вќЊ | Abbreviated label for summary views |
| `sdc-questionnaire-collapsible` | вќЊ | Group renders collapsed by default (`default-closed`) or expanded (`default-open`). Currently all groups start expanded. |
| `sdc-questionnaire-openLabel` | вќЊ | Custom label for the free-text "other" option in `open-choice` items (default: "Other"). |

### SDC extensions вЂ” population and extraction (out of scope)
...
| Version | Status |
|---|---|
| R4 | вњ… Fully supported |
| R4B / R5 | рџ”§ Partial вЂ” most fields overlap; `answerConstraint` and `disabledDisplay` are R4B/R5 native (R4 backport extension handled for `disabledDisplay`) |
| STU3 | вќЊ Not tested; may partially import |
