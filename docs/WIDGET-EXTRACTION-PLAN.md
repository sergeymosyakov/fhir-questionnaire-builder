...
| E | Multi-instance isolation: per-session bus everywhere, drop `document` from render path; session config provider; two-widgets-on-one-page e2e | ✅ Done | lint ✓ · vitest 1658 ✓ · widget e2e 7/7 · broad e2e 61/61 | **Render path off `document`:** `rc.bus` (set by PreviewForm); nodes wire preview listeners (`PREVIEW_NAVIGATE_TO`, `REFRESH_CALC_BADGES`, `COLLAPSE/EXPAND_ALL_PREVIEW`, `BUILDER_NAVIGATE`) on `rc.bus` via `_ensureBusListeners`; **`notifyChanged(bus)`** + all ~25 `buildControl` handlers dispatch on `ctx.bus` (fixes live-input recompute); preview→builder nav on `rc.bus` gated by `rc.showNavBtn`. `COPY_TO_NODES`/`CLIPBOARD_CHANGED`/builder events + raw outside-click stay on `document` (builder-only). **Session config:** `config.fhirBaseUrl`/`config.corsProxy` flow via `session.config` (PreviewForm resolves session→global `serverConfig` fallback; `reference-node`/`searchFhir`/`proxiedUrl`/`_populate`). **Config flags** (default off; widget opt-in): `search`, `validation`, `explain`, `navButton`, `tooltips`. **Explain in widget** works (`Modal` self-mounts to body; needs `modals.css` in the manifest); plain numeric calc values + standard-`enableWhen` hints are clickable → Explain / “why hidden” audit modal (`explainModal.showAudit`). **Tooltips** via `config.tooltips` (idempotent page-wide `tooltip.init`, storage-tolerant). **Isolation e2e:** live-input in one widget recomputes only itself, sibling untouched. ~~Deferred: per-widget terminology server (cache keyed by `(vsUrl,serverUrl)` = safe to share; `$expand` not in widget import path).~~ **Resolved:** `TerminologyService` now takes its config source via constructor DI; the widget builds its own instance from `config.terminology` and wires `expandAll()` into the load path (see CONTEXT.md). |
| F | Packaging: `js/renderer/index.js` entry, `widget-demo.html`, docs (CONTEXT / docs-site "Embed the renderer" / ROADMAP) | ✅ Done | lint ✓ · vitest 1658 ✓ · widget e2e 7/7 · `build:widget` ✓ | `js/renderer/index.js` ESM entry exports `QuestionnaireRenderer`; esbuild `build:widget` → `dist/questionnaire-widget.{js,global.js,css}` (357/376/44 kb); `widget-demo.html` = 3 isolated widgets (search+validation+explain+tooltips) over `annual-health-check`; columns get their own scroll (equal height, no page scroll). Docs: CONTEXT (core/renderer/widget manifest rows), ROADMAP (near-term entry), docs-site **`embed-renderer.md`** ("Embed the renderer" how-to). `builder-helpers.js` moved js/nodes→js/builder. |
