...
1. [ ] **StructureMap execution for `$populate`** — execute `sdc-questionnaire-sourceStructureMap` in-browser to pre-populate a QuestionnaireResponse from source resources. `targetStructureMap`/`$extract` is now implemented client-side (Save ▾ → StructureMap Extract) via [fhir-structuremap-js](https://github.com/sergeymosyakov/fhir-structuremap-js), a real FML/StructureMap engine vendored in `lib/`. Only a `#id` reference to a StructureMap added via the Contained Resources panel is resolvable — no server to fetch an external canonical URL. Tracked in [issue #59](https://github.com/sergeymosyakov/fhir-questionnaire-builder/issues/59).
