...
- [x] **[HIGH CRITICAL] Add `system` field to answer options** — Implemented: `_optionSystems` map; Answer Type modal options editor now has a System column (URI input per row); exported as `valueCoding.system` in the simple options path; preserved from `_rawAnswerOptions.valueCoding.system` in the raw path. Eliminates HAPI `"A code with no system has no defined meaning"` warnings for options with a system set.
