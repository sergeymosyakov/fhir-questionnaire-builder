...
4. [x] **Renderer maturity + accessibility audit** — incremental. **Done:** dependency-free a11y e2e over the preview (`tests/e2e/a11y.spec.js`) — every preview form control receives an accessible name (`aria-label` from item text) via `PreviewForm._applyA11yLabels`, AA colour-contrast on the item prefix and "optional" badges; **modal focus management** (`tests/e2e/modal-focus.spec.js`) — `role="dialog"` + `aria-modal`, focus moves into the dialog on open, Tab focus-trap, focus returns to the opener on close (`js/ui/modals/modal-base.js`); builder action links are keyboard-accessible (`role="button"`, `tabindex=0`, Enter/Space via `makeActionLink`); **custom-select ARIA** (`tests/e2e/custom-select-a11y.spec.js`) — `createCustomSelect` and the choice-node preview select expose `role=combobox`/`listbox`/`option`, `aria-expanded`, `aria-selected`; `createCustomSelect` adds full keyboard navigation (Arrow keys + `aria-activedescendant`, Enter, Home/End). **Screen-reader error announcements** — inline control errors (`.ctrl-err`) are `role="alert"` / `aria-live="assertive"` and the PASS/FAIL status badge is an `aria-live="polite"` region (`tests/e2e/validation-aria-live.spec.js`). Both the shared `createCustomSelect` and the choice-node preview dropdown support Arrow/Home/End/Enter keyboard navigation with `aria-activedescendant`. **Builder tree keyboard access** (`tests/e2e/builder-tree-a11y.spec.js`) — node title click-to-edit is `role="button"` (Enter/Space opens the editor); the ⚙ gear button exposes `aria-haspopup="menu"`/`aria-expanded`; gear menu items are `role="menuitem"` and keyboard-reachable (`tabindex=0`, Enter/Space, Escape closes and restores focus). Collapse toggles were already native `<button>`s. A formal external a11y audit and production hardening remain out of scope for this prototype.
