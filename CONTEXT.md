...
- **Shared modal system** — all dialogs (Variables, Show When, Patient Context, Validate) use the same `.modal-*` CSS classes; structural styles live once in `css/modals.css`; per-modal overrides (z-index, width, body padding) via `#id` selectors; design tokens `--c-hover` and `--c-text-1` added to `css/styles.css`; modal title pattern: `.modal-title-label` (bold) + `.modal-title-subject` (muted, right of label)
...
- **Shared modal system** — all dialogs (Variables, Show When, Patient Context, Validate) use the same `.modal-backdrop`, `.modal-box`, `.modal-header`, `.modal-close`, `.modal-body`, `.modal-footer`, `.modal-btn`, `.modal-btn--cancel`, `.modal-btn--apply` classes from `css/modals.css`; per-modal overrides (z-index, width, body padding) via `#id` CSS selectors only; design tokens `--c-hover` (`#f0f2f5`) and `--c-text-1` added to `css/styles.css`
