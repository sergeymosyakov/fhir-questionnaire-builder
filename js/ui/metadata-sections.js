// ── Questionnaire Properties modal — body renderer ───────────────────────
...
export function renderMetaSections(container, pending) {
  ...
  let derivedOpen = pending.derivedFrom.length > 0;
  ...
  const _setDerivedLabel = () => {
    const count = pending.derivedFrom.filter(u => u.trim()).length;
  ...
  const _renderDerived = () => {
    ...
    if (pending.derivedFrom.length === 0) {
    ...
    pending.derivedFrom.forEach((url, idx) => {
      const row = document.createElement('div');
      ...
      inp.oninput = () => { pending.derivedFrom[idx] = inp.value; _setDerivedLabel(); };
      const removeBtn = document.createElement('button');
      removeBtn.type = 'button';
      removeBtn.className = 'codes-remove-btn';
      removeBtn.textContent = '\u00D7';
      removeBtn.dataset.testid = `meta-derived-remove-${idx}`;
      removeBtn.onclick = () => { pending.derivedFrom.splice(idx, 1); _renderDerived(); _setDerivedLabel(); };
  ...
  let replacesOpen = pending.replaces.length > 0;
  ...
  const _setReplacesLabel = () => {
    const count = pending.replaces.filter(u => u.trim()).length;
    const badge = count ? ` (${count})` : '';
  ...
  const _renderReplaces = () => {
    ...
    pending.replaces.forEach((url, idx) => {
      ...
      inp.oninput = () => { pending.replaces[idx] = inp.value; _setReplacesLabel(); };
      const removeBtn = document.createElement('button');
      removeBtn.type = 'button';
      removeBtn.className = 'codes-remove-btn';
      removeBtn.textContent = '\u00D7';
      removeBtn.dataset.testid = `meta-replaces-remove-${idx}`;
      removeBtn.onclick = () => { pending.replaces.splice(idx, 1); _renderReplaces(); _setReplacesLabel(); };
      row.append(inp, removeBtn);
  ...
  let idOpen = pending.identifiers.length > 0;
  ...
  const _setIdLabel = () => {
    const count = pending.identifiers.filter(i => i.system?.trim() || i.value?.trim()).length;
    const badge = count ? ` (${count})` : '';
  ...
  const _renderIdentifiers = () => {
    ...
    pending.identifiers.forEach((ident, idx) => {
      ...
      removeBtn.onclick = () => { pending.identifiers.splice(idx, 1); _renderIdentifiers(); _setIdLabel(); };
    ...
    addBtn.onclick = () => {
      pending.identifiers.push({ system: '', value: '' });
      idOpen = true;
  ...
  let contactOpen = pending.contacts.length > 0;
  ...
  const _setContactLabel = () => {
    const count = pending.contacts.filter(c => c.name?.trim() || c.telecom?.some(t => t.value?.trim())).length;
    const badge = count ? ` (${count})` : '';
  ...
  const _renderContacts = () => {
    ...
    pending.contacts.forEach((contact, ci) => {
      ...
      removeContactBtn.onclick = () => { pending.contacts.splice(ci, 1); _renderContacts(); _setContactLabel(); };
  ...
  let jurOpen = pending.jurisdictions.length > 0;
  ...
  const _renderJurisdictions = () => {
    ...
    pending.jurisdictions.forEach((jur, idx) => {
      ...
      removeBtn.onclick = () => { pending.jurisdictions.splice(idx, 1); _renderJurisdictions(); _setJurLabel(); };
  ...
  const _setJurLabel = () => {
    const count = pending.jurisdictions.filter(jur => jur.coding?.[0]?.code?.trim()).length;
    const badge = count ? ` (${count})` : '';
  ...
  const _renderProfile = () => {
    ...
    pending.metaProfile.forEach((url, idx) => {
      ...
      rm.onclick = () => { pending.metaProfile.splice(idx, 1); _renderProfile(); };
      row.append(inp, rm);
  ...
  profileAddBtn.onclick = () => { pending.metaProfile.push(''); _renderProfile(); };
  ...
  renderCodesEditor(pending.codes, codesBody, 'meta-code');
