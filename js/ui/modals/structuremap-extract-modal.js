// ── StructureMap Extract Modal ────────────────────────────────────────────────
// Shows the result of executing the Questionnaire's targetStructureMap: list of
// extracted resources + Download Bundle button. Mirrors def-extract-modal.js.
...
import { structureMapExtract } from '../../fhir/sdc-structuremap-extract.js';
import { EventState }   from '../../events.js';

export class StructureMapExtractModal extends Modal {
  constructor() {
    super({ applyLabel: 'Download Bundle', cancelLabel: 'Close', maxWidth: '540px' });
    this.title.textContent = 'StructureMap-based Extraction';
    this._result = null;
  }

  getName() { return 'structureMapExtract'; }
  ...
  run() {
    ...
    this._result = structureMapExtract(questJson, qr);
  ...
  _renderBody() {
    ...
    for (const entry of bundle.entry) {
      ...
      title.textContent = r.resourceType || '(untyped)';
  ...
  _apply() {
    ...
    downloadJSON(this._result.bundle, 'structuremap-extracted-resources.json');
    this.close();
  }
...
export const structureMapExtractModal = new StructureMapExtractModal();

// Self-wire to AppEvents
if (typeof document !== 'undefined') {
  document.addEventListener(AppEvents.STRUCTUREMAP_EXTRACT_REQUESTED, () => structureMapExtractModal.run());
}
