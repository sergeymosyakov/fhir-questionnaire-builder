...
function walkNode(node, linkIdMap, translations, fp, depth, out, contained) {
  out.push({
    ...
    choiceOrientation: node._choiceOrientation || null,
    displayCategory: node._displayCategory || null,
    usageMode: node._usageMode || null,
    signatureRequired: node._signatureRequired?.length ? node._signatureRequired : null,
    definition: node._definition || null,
    baseType: node._baseType || null,
    fhirType: node._fhirType || null,
    answerSource: {
    ...
    minValue: node._minValue ?? null,
    maxValue: node._maxValue ?? null,
    sliderStep: node._sliderStep ?? null,
    answerConstraint: node._answerConstraint || null,
    codes: node._codes?.length ? node._codes : null,
    supportLinks: node._supportLinks?.length ? node._supportLinks : null,
    maxFileSizeMB: node._maxFileSizeMB ?? null,
    mimeTypes: node._mimeTypes?.length ? node._mimeTypes : null,
    referenceResourceType: node.referenceResource || null,
    referenceProfiles: node._referenceProfiles?.length ? node._referenceProfiles : null,
    referenceFilter: node._referenceFilter || null,
    unit: node.quantityUnit || null,
    unitOptions: node._unitOptions?.length ? node._unitOptions : null,
    unitValueSet: node._unitValueSet || null,
    regex: node._regex || null,
    preferredTermServer: node._preferredTermServer || null,
    unknownExtensionCount: node._unknownExtensions?.length || null,
