// ── E2E: XHTML translation support ────────────────────────────────────────────
// Verifies that items with rendering-xhtml extension:
//   1. Send HTML (not plain text) to the translate modal review table
//   2. After applying, the translated XHTML is rendered in the preview
//   3. The FHIR export includes the xhtml-translations custom extension
//
// Fixture: sampledata/rendering-xhtml-demo.fhir.json
// Items with _renderXhtml: intro, grp-personal, grp-symptoms
//
...
async function stubTranslateAPI(page) {
  await page.route('**/translate.googleapis.com/**', async route => {
    ...
    // Prefix each newline-delimited segment with [ES] 
    const translated = q.split('\n').map(l => (l.trim() ? `[ES] ${l}` : l)).join('\n');
